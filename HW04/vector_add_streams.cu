...
int main()
{
    ...
    cudaMalloc(&d_B, size);
    cudaMalloc(&d_C, size);
    cudaStream_t stream 1, stream 2;
    ...
    vector_add<<<blocks_half, threads, 0, stream 2>>>(d_A+half, d_B+half, d_ +half, half);
    cudaMemcpyAsync(C, d_C, half_size, cudaMemcpyDeviceToHost, stream 1);
    cudaMemcpyAsync(C + half, d_C + half, half_size,cudaMemcpyDeviceToHost, stream 2);
    cudaStreamSyncronize(stream 1);
    cudaStreamSyncronize(stream 2);
